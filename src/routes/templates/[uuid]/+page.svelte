<script lang="ts">
	...
	let editData = $state({
		...
		successPattern: template.successPattern
	});
...
<main class="p-4 sm:p-6 lg:p-8 bg-gray-900 text-gray-300 min-h-screen font-sans">
	<div class="max-w-4xl mx-auto">
		...
		<div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
			...
			<div class="flex items-center gap-2 self-start sm:self-center">
				<button
					...
					title={!usage.isDeletable ? 'This template is in use and cannot be deleted.' : 'Delete template'}
				>
					<Trash2 size={16} />
					<span>Delete</span>
				</button>
		...
		<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
			<form onsubmit|preventDefault={handleUpdate} class="space-y-6">
				<div>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
					<div>
						...
						<p class="text-xs text-gray-500 mt-1">
							Path to file/folder to be zipped, relative to repo root.
						</p>
					</div>
					<div>
				...
				<div>
					...
					/>
					<p class="text-xs text-gray-500 mt-1">
						A case-insensitive regex pattern to match in logs to confirm success.
					</p>
				</div>
				...
				{#if !usage.isDeletable}
					<div class="bg-yellow-500/10 text-yellow-300 p-4 rounded-md text-sm flex gap-3">
						<Info size={20} class="flex-shrink-0 mt-0.5" />
						<div>
							<h3 class="font-semibold">Template in Use</h3>
							<p class="mt-1">
								This template is currently used by {usage.usedBy.length}
								{usage.usedBy.length === 1 ? 'repository' : 'repositories'} and cannot be deleted.
								You must assign these repositories to a different template before deletion.
							</p>
							<ul class="list-disc pl-5 mt-2">
								{#each usage.usedBy as repo}
									<li>
										<a href="/repos/{repo.uuid}" class="font-medium hover:underline"
											>{repo.name}</a
										>
									</li>
								{/each}
	...
	>
		...
		>
			...
			<p class="text-gray-400 mt-4">
				Are you sure you want to delete the template <strong class="text-white"
