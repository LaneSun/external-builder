<script lang="ts">
	...
	async function handleSubmit() {
		...
		if (!newTemplate.name || !newTemplate.script || !newTemplate.timeout || !newTemplate.resultPath || !newTemplate.successPattern) {
			formError = 'Name, script, timeout, result path, and success pattern are required.';
			isSubmitting = false;
			return;
		}
...
<main class="p-4 sm:p-6 lg:p-8 bg-gray-900 text-gray-300 min-h-screen font-sans">
	<div class="max-w-7xl mx-auto">
		<div class="flex justify-between items-center mb-6">
			...
			<button
				onclick={() => showAddForm ? (showAddForm = false) : openAddForm()}
		...
		{#if showAddForm}
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6 mb-8">
				<h2 class="text-xl font-semibold mb-4 text-white">Add a New Build Template</h2>
				<form onsubmit|preventDefault={handleSubmit} class="space-y-4">
                    ...
                    <div>
						<label for="script" class="block text-sm font-medium text-gray-400 mb-1">
						...
						></textarea>
                        <p class="text-xs text-gray-500 mt-1">Enter commands as they would be run in `cmd.exe`.</p>
					</div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="resultPath" class="block text-sm font-medium text-gray-400 mb-1">
                                Artifact Path
                            </label>
                            <input
                                id="resultPath"
                                ...
                                class="w-full bg-gray-900 border border-gray-600 rounded-md px-3 py-2 text-white placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            />
                            <p class="text-xs text-gray-500 mt-1">Path to file/folder to be zipped, relative to repo root.</p>
                        </div>
                        <div>
                    ...
                    <div>
						<label for="successPattern" class="block text-sm font-medium text-gray-400 mb-1">
						...
						<input
							...
							class="w-full bg-gray-900 border border-gray-600 rounded-md px-3 py-2 text-white placeholder-gray-500 font-mono text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
						/>
                        <p class="text-xs text-gray-500 mt-1">A case-insensitive regex pattern to match in the build logs to confirm success.</p>
					</div>
				...
				{#each templates as template (template.uuid)}
					...
					>
						<div class="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
							<div class="flex-grow">
								<p class="font-bold text-lg text-white">{template.name}</p>
								<p class="text-sm text-gray-400">{template.description || 'No description'}</p>
							</div>
						...
						<div class="mt-4 border-t border-gray-700 pt-3">
                            <p class="font-mono text-xs bg-black/30 p-2 rounded-md text-gray-300 whitespace-pre-wrap">{template.script}</p>
                        </div>
					</a>
				{/each}
			{:else}
				...
				>
					...
					<h3 class="mt-4 text-lg font-semibold text-white">No Build Templates</h3>
