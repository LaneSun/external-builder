<script lang="ts">
	...
	const duration = $derived(calculateDuration(task.startedAt, task.finishedAt));
</script>
...
<main class="p-4 sm:p-6 lg:p-8 bg-gray-900 text-gray-300 min-h-screen font-sans">
	<div class="max-w-7xl mx-auto">
		...
		<div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
			<div>
				<h1 class="text-3xl font-bold text-white">Build Task Details</h1>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-5">
				<h2 class="text-sm font-medium text-gray-400 mb-2 flex items-center gap-2">
					<CheckCircle size={16} />
					Status
				</h2>
				<div
			...
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-5">
				...
				<p class="text-lg font-semibold text-white">{formatTime(task.createdAt)}</p>
			</div>
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-5">
				<h2 class="text-sm font-medium text-gray-400 mb-2 flex items-center gap-2">
					<StopCircle size={16} />
				...
				<p class="text-lg font-semibold text-white">{formatTime(task.finishedAt)}</p>
			</div>
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-5">
				<h2 class="text-sm font-medium text-gray-400 mb-2 flex items-center gap-2">
					<Timer size={16} />
		...
		{#if task.status === 'failed' && task.error}
			<div class="bg-red-900/50 border border-red-500/30 rounded-xl p-6 mb-8">
				<h2 class="text-xl font-semibold text-red-300 flex items-center gap-3">
					<XCircle size={24} />
					Build Failed
				</h2>
				<p class="font-mono bg-black/20 p-3 rounded-md mt-3 text-red-300">{task.error}</p>
			</div>
		{/if}

		<!-- Build Logs -->
		<div class="bg-gray-800/50 border border-gray-700 rounded-xl">
			<h2 class="text-xl font-semibold text-white p-6 border-b border-gray-700">Build Logs</h2>
			<div class="p-6">
				<pre
					class="bg-black/50 p-4 rounded-lg text-sm text-gray-300 font-mono whitespace-pre-wrap break-words max-h-[600px] overflow-y-auto"
				>{task.logs}</pre>
			</div>
		</div>
	</div>
