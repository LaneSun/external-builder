<script lang="ts">
	...
	let newRepo = $state({
		...
		gitlabUrl: '',
		branch: 'main',
		templateUuid: templates.length > 0 ? templates[0].uuid : ''
	});
	...
	async function handleSubmit() {
		...
		if (!newRepo.name || !newRepo.gitlabUrl || !newRepo.branch || !newRepo.templateUuid) {
	...
	<div class="max-w-7xl mx-auto">
		...
		{#if showAddForm}
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6 mb-8">
				<h2 class="text-xl font-semibold mb-4 text-white">Add a New Repository</h2>
				<form onsubmit|preventDefault={handleSubmit} class="space-y-4">
					...
					<div>
						<label for="gitlabUrl" class="block text-sm font-medium text-gray-400 mb-1">
		...
		<div class="space-y-4">
				...
				{#each repos as repo (repo.uuid)}
					...
					>
						<div class="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
							<div class="flex-grow">
								<p class="font-bold text-lg text-white">{repo.name}</p>
								<p class="text-sm text-gray-400 break-all">{repo.gitlabUrl}</p>
							</div>
							<div
							...
							>
								<svelte:component
									...
									size={14}
									class={repo.status === 'building' || repo.status === 'cloning'
										? 'animate-spin'
										: ''}
								/>
								<span class="capitalize font-medium">{repo.status}</span>
							</div>
						</div>
						<div class="mt-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-sm">
							<div class="flex items-center gap-2 text-gray-400">
								...
								<span>Branch: <span class="font-semibold text-gray-300">{repo.branch}</span></span>
							</div>
							<div class="flex items-center gap-2 text-gray-400">
								<GitCommit size={16} class="text-gray-500" />
								<span
									>Last Updated:
									<span class="font-semibold text-gray-300">{formatTime(repo.updatedAt)}</span></span
								>
							</div>
						</div>
					</a>
			...
			{:else}
				...
				>
					...
					<h3 class="mt-4 text-lg font-semibold text-white">No Repositories Tracked</h3>
